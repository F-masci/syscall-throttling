...
/* This routine traverses the page table to check * *  if a virtual page is mapped into a frame */
...
int sys_vtpmo(unsigned long vaddr){
	...
	target_address = (void*)vaddr; *		/* fixing the address to use for page table access */	 *
	AUDIT{
	printk("%s: ------------------------------\n",MODNAME);
	}
	...
	if ((ulong)pde[PDE(target_address)].pmd & LH_MAPPING){ *		AUDIT
		printk("%s: PDE region mapped to large page\n",MODNAME);
		frame_addr = (ulong)(pde[PDE(target_address)].pmd) & PT_ADDRESS_MASK; *		AUDIT
